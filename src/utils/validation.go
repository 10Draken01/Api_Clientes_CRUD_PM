...
func ValidateCliente(cliente models.Cliente) map[string][]string {
	...
	switch v := cliente.Character_Icon.(type) {
		case string:
			icon, err := strconv.Atoi(v)
			if err == nil {
				if icon < 0 || icon > 9 {
					errores["Character_Icon"] = append(errores["Character_Icon"], "Debe estar entre 0 y 9")
				}
				cliente.Character_Icon = icon
			} else {
				errores["Character_Icon"] = append(errores["Character_Icon"], "Debe ser un número entero")
			}
